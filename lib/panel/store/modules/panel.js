...
const isSameGroup = (eventGroup, event) => {
  ...
  if (
    (_.get(lastEvent.original, 'selector.xpath') !== _.get(event.original, 'selector.xpath')) &&
    !(
      _.get(lastEvent.original, 'element.tagName') === 'label' &&
      _.get(lastEvent.original, 'element.attr.for') === _.get(event.original, 'selector.id')
    )
  ) return false;

  if (_.get(lastEvent.original, 'event') === 'click' && _.get(event.original, 'event') === 'change') return true;
